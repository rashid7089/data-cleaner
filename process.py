...
def cleaningProcess(file_path, data_types={}):
    try:
        ...
        for c in null_columns:
            if df[c].dtype!='object':
                ...
                value = value[0]
            df[c].fillna(value,inplace=True)
        # return dictionary includes the data
        return df.to_dict()

    except:
        return {"message":"Sorry, the given path is incorrect. Try again."}
...
#     # def set_types_by_dict(self, cat_type):
